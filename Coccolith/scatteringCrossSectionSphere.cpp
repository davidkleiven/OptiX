...
double Nx = 2.0*(4.0*radius+pmlThick);
double Ny = 2.0*(4.0*radius+pmlThick);
double Nz = 2.0*(4.0*radius+pmlThick);
...
{
  double resolution = 5.0; // pixels per distance
  ...
  f.add_volume_source( fieldComp, src, srcVol, amplitude );
    ...
    meep::vec refCrn1( Nx/2.0-radius-1.0, pmlThick, pmlThick );
    meep::vec refCrn2( Nx/2.0-radius-1.0, Ny-pmlThick-1.0, Nz-pmlThick-1.0);
    meep::volume refPlaneVol(refCrn1, refCrn2);
    meep::vec boxEndCrn( Nx/2.0+radius+1.0, Ny-pmlThick-1.0, Nz-pmlThick-1.0);
    meep::vec transPlaneCrn( Nx/2.0+radius+1.0, pmlThick, pmlThick );
    meep::volume fluxBoxVol( refCrn1, boxEndCrn );
    meep::volume transPlaneVol( transPlaneCrn, boxEndCrn );
    meep::dft_flux refPlane = f.add_dft_flux_plane( refPlaneVol, freq-fwidth/2.0, freq+fwidth/2.0, numberOfFreq );
    meep::dft_flux transPlane = f.add_dft_flux_plane( transPlaneVol, freq-fwidth/2.0, freq+fwidth/2.0, numberOfFreq );
  ...
  while ( f.time() < endTime)
  {
    ...
    if ( overwriteFile )
    {
      ...
      file->write("transFluxRef", 1, &Nfreq, fluxRef, false );
      file->prevent_deadlock();
    ...
    else
    {
      ...
      double *fluxTrans = transPlane.flux();
      file->write("transFluxScat", 1, &Nfreq, fluxRef, false );
      file->prevent_deadlock();
